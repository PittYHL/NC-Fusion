...
* **External software:** PySCF for molecular Hamiltonians, the Synthetiq
  ...
  for the Pauli-based T-gate optimizer comparison.
...
* **Expected outputs:** `metrics.csv`, `manifest.json`, and, when requested,
  generated QASM under `circuits/`.

## Access to the artifact

From the repository root, the artifact is in this project. The paper is
`NC_Fusion.pdf`. No credentials or network service are required for the smoke
...
1. Read `configs/paper.json` to select a paper section and its exact benchmark
   and parameter set.
...
5. Run the desired configured section, preserving its output directory and
   `manifest.json`.
6. Compare the main metrics with `results/reference/table4.csv` using the
...
PYTHONPATH=src python -m ncfusion run table4 \
  --benchmark LiH --method gridsyn --output results/runs/check-lih

# Main comparison
PYTHONPATH=src python -m ncfusion run table4 \
  --output results/runs/table4
```
...
`results/reference/table4.csv` for evaluator comparison. The full artifact
